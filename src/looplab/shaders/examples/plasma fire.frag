...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float dist = 3.5-sin(time*1.0)/1.89;
    ...
    p += sin(p.yx*4.0+vec2(.2,-.3)*sin(time))*0.04;
    p += sin(p.yx*8.0+vec2(.6,+.1)*sin(time))*0.01;
    
    p.x -= time/1.0;
    float q = fbm(p - time*1.0+1.0*sin(time+0.5)/2.0);
    float qb = fbm(p - time*1.0+0.1*cos(time)/2.0);
    float q2 = fbm(p - time*1.0 - 5.0*cos(time)/2.0) - 6.0;
    float q3 = fbm(p - time*1.0 - 10.0*cos(time)/15.0)-4.0;
    float q4 = fbm(p - time*1.0 - 20.0*sin(time)/14.0)+2.0;
