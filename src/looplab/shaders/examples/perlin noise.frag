...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 p = uv / 2.0;
    vec3 p3 = vec3(p, LOOP_TIME*1.0);
    
    float value;
    
    value = simplex3d_fractal(p3*10.0);
    
    float force = pow(iForce/12.0,2.)+0.2;
    float force2 = pow(iForce2/5.5,2.)+0.1;
    value = force + force2*value;    
