...
float audio_ampl( in sampler2D channel, in float t) { return texture( channel, vec2(t, 0.75) ).x*pow(iForce/5.5,2.0); }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec3 disc_color  = 1.0 * base_color;
    vec3 wave_color  = 1.0 * base_color;
    vec3 flash_color = 0.05 * base_color;
    
    
    float sample2 = audio_ampl(iAudioSamples, clamped_dist);
    float sample3 = audio_ampl(iAudioSamples, arclength);
    
    float disp_dist = smoothstep(-0.1, 3.9, (sample3-dist2));
    disp_dist *=  pow(disp_dist*50.0, 5.);
    ...
    if(iComplexity > 3) color += disc_color  * smoothstep(0.5, 1.0, sample2) * (1.0 - clamped_dist);
    if(iComplexity > 2) color += flash_color * smoothstep(0.5, 1.0, sample3) * clamped_dist;
