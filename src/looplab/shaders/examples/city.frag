...
float power = iForce < 5.5 ? pow(iForce/5.5, 0.5) : pow(iForce/5.5, 4.2);
...
vec3 textureWall(vec2 pos, vec2 maxPos, vec2 squarer,float s,float height,float dist,vec3 rayDir,vec3 norm){
    ...
    if ((random <0.05*power*(3.5-2.5*floorFactor))||(windows>0.65/power)){
...
vec3 cars(vec2 squarer, vec2 pos, float dist,float level){
    ...
    for (int j=0;j<10; j++){
        ...
        if(fract(random*5.0/i)>carNumber){color += car1*carInten*smoothstep(carRadis,0.0,length(pos - vec2(fract(i+LOOP_TIME/4.0),0.025)));}
        
        if(fract(random*10.0/i)>carNumber){color += car2*carInten*smoothstep(carRadis,0.0,length(pos - vec2(fract(i-LOOP_TIME/4.0),0.975)));}
        if(color.x>0.0) break;
    }
    for (int j=0;j<10; j++){
        float i= 0.03+float(j)*0.094;
        if(fract(random*5.0/i)>carNumber){color += car2*carInten*smoothstep(carRadis,0.0,length(pos - vec2(0.025,fract(i+LOOP_TIME/4.0))));}
        if(fract(random*10.0/i)>carNumber){color += car1*carInten*smoothstep(carRadis,0.0,length(pos - vec2(0.975,fract(i-LOOP_TIME/4.0))));}
            if(color.x>0.0) break;

    }
    for (int j=0;j<10; j++){
        float i = 0.03+0.047+float(j)*0.094;
        if(fract(random*100.0/i)>carNumber){color += car1*carInten*smoothstep(carRadis,0.0,length(pos - vec2(fract(i+LOOP_TIME/4.0),0.045)));}
        if(fract(random*1000.0/i)>carNumber){color += car2*carInten*smoothstep(carRadis,0.0,length(pos - vec2(fract(i-LOOP_TIME/4.0),0.955)));}
            if(color.x>0.0) break;

    }
    for (int j=0;j<10; j++){
        float i = 0.03+0.047+float(j)*0.094;
        if(fract(random*100.0/i)>carNumber){color += car2*carInten*smoothstep(carRadis,0.0,length(pos - vec2(0.045,fract(i+LOOP_TIME/4.0))));}
        if(fract(random*1000.0/i)>carNumber){color += car1*carInten*smoothstep(carRadis,0.0,length(pos - vec2(0.955,fract(i-LOOP_TIME/4.0))));}
            if(color.x>0.0) break;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec2 pos = (fragCoord.xy / iResolution.xy-0.5)*2.0;
