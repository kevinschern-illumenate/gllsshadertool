...
#define max_floor_bounce 6
#define max_wall_bounce 15
...
#define samples (pow(0.+iComplexity, 2.0)*3.0)
...
#define time_per_simulation 3.0
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float t = LOOP_TIME*1.0 + time_offset;
    ...
    for (int sampleIndex = 0; sampleIndex<samples; sampleIndex++)
    {
        ...
        vec3 p = (
            physics(
                vec3(pos.x,pos.y+12.0,8.0), //initial position
        ...
        temp/=400./(300.0 / (iForce/5.5)); //adjust the size of the particles
        float s = pow(dot(temp,temp), 2.);
        //s-=1.0;
        s*=400.*.05;
        //s = min(1.0,max(.0,s));
    ...
    vec3 color = part_color*acc*brightness;
    ...
    color -= length(uv)*.005;
    color = cc(color,.9,.9);
    color = vec3(1.5)*color/(vec3(1.0)+color);
