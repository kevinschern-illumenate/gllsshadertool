...
#define time (LOOP_TIME*10.0)
#define iBaseHueRad 2.73
...
vec4 aurora(vec3 ro, vec3 rd)
{
    vec4 col = vec4(0);
    vec4 avgCol = vec4(0);
    float count = min(50.,iComplexity * 14.);
    for(float i=0.;i<count;i++)
    {
        float of = 0.007*hash21(gl_FragCoord.xy)*smoothstep(0.,15., i);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    rd.yz *= mm2(mo.y);
    rd.xz *= mm2(mo.x + (time*1.0));
    
    vec3 col = vec3(0.);
    vec3 brd = rd;
    float power = pow(iForce/5., 2.2);
    float fade = (smoothstep(0.,0.01,abs(brd.y))*0.1+0.9)*power;
    
    col = bg(rd)*fade;
    
    if (rd.y > 0.){
        vec4 aur = smoothstep(0.,1.5,aurora(ro,rd))*fade;
        if(iComplexity>4)col += stars(rd);
    ...
    else //Reflections
    {
        ...
        if(iComplexity>4)col += stars(rd)*0.2;
        col = col*(1.-aur.a) + aur.rgb*power;
        if(iComplexity>4)
        {
