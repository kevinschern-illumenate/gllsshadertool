...
float lightning2(vec2 ls, vec2 le, float intensity,vec2 uv)
{
    ...
    for(int i = 0;i < LIGHTNING_PARTS;i++)
    {
        ...
        float lh1 = lhash(ltemp1.y, LOOP_TIME*1.0);
        float lh2 = lhash(ltemp2.y, LOOP_TIME*1.0);
        ltemp1.x += lh1;
        ltemp2.x += lh2;
...
float lightning(vec2 ls, vec2 le, float intensity,vec2 uv)
{
    ...
    for(int i = 0;i < LIGHTNING_PARTS;i++)
    {
        ...
        float lh1 = lhash(ltemp1.y + ltemp1.x, LOOP_TIME*1.0);
        float lh2 = lhash(ltemp2.y + ltemp2.x, LOOP_TIME*1.0);
        ltemp1.x += lh1;
        ltemp2.x += lh2;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 cad = uv + vec2(LOOP_TIME*1.0,0.0);     
    float p = fbm(cad); 
    p = 1.0 - abs(p * 2.0 - 1.0);
    ...
    //ls.x = mod(-LOOP_TIME*1.0,2.0)-1.0;
    //le.x = mod(-LOOP_TIME*1.0,2.0)-1.0;
    
    float f = 1.0;
    for(int i = 0; i < 60*iComplexity; i++ )
    {
        //Lightning Seeds
        float h = hash(vec2(sqrt(float(i)),floor((LOOP_TIME*1.0) * 7.0 + (float(i) / 10.0)) * float(i)));
        ls.x = hash(h) * 3.0  - mod(LOOP_TIME*1.0,3.0);
