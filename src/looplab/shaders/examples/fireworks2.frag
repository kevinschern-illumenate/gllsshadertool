...
#define SPARKS (iNbItems2*2)
#define FIREWORKS (iNbItems + 0.0)
...
vec2 circularSpark(float i, float noiseId, float noiseSpark) {
    ...
    float x = sin(a + LOOP_TIME*((noiseId-.5)*3.));
    float y = cos(a + LOOP_TIME*(fract(noiseId*4567.332) - .5)*2.);
    ...
    if (et > rocketTime && et < (rocketTime + explodeTime)) {
        ...
        for(int i = 0 ; i < SPARKS ; i++) {
                ...
                if (t > 0.) {
                    ...
                    vec3 c = (iForce / 100.0 / d) * sparkColor;
                    col += c * fade;
                }
            }
    }
