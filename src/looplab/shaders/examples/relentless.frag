...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// ray origin
	vec3 ro = 0.7 * vec3(cos(1.0*time), 0.0, sin(1.0*time));
	ro.y = cos(1.0*time) * 0.3 + 0.65;
	// camera look at
	...
	for(int i = 0; i < 4; i ++)
	{
		...
		if(its.x > 0.0)
		{
			...
			if(i == 3)
			{
				float crd = 0.0;//fract(time*1.0) * 50.0 - 25.0;
				float fxi = cos(vo.x * 0.2 + time*2.0);//abs(crd - vo.x);
