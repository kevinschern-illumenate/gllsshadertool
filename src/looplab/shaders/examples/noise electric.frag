...
float dualfbm(in vec2 p)
{
	...
	vec2 p2 = p*.7;
	vec2 basis = vec2(fbm(p2-time*2.0),fbm(p2+time*2.0));
	basis = (basis-.5)*.2;
	p += basis;
	
	//coloring
	return fbm(p*makem2(time*1.0));
}
...
float circ(vec2 p) 
{
	float r = length(p);
	r = log(sqrt(r));
	float power = 3.0 / pow(iForce/5.5, 2.);
	return abs(mod(r*4.,tau)-3.14)*power+.2;
