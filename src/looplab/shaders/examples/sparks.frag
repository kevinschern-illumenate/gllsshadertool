...
//Shader License: CC BY 3.0 https://www.shadertoy.com/view/wl2Gzc
//Author: Jan Mróz (jaszunio15)
#define iBaseHueRad 0.4563 
#define PI 3.1415927
#define TWO_PI 6.283185

#define ANIMATION_SPEED 0.75
#define MOVEMENT_SPEED 0.5
#define MOVEMENT_DIRECTION vec2(0.7, -1.0)

#define PARTICLE_SIZE (iForce / 280.0)
...
vec3 SPARK_COLOR = mColorMode != 1 ? vec3(1.0, 0.4, 0.05) * 1.5 : vec3(1.0);
#define BLOOM_COLOR vec3(1.0, 0.4, 0.05) * 0.8


#define SIZE_MOD 1.05
#define ALPHA_MOD 0.9
#define LAYERS_COUNT (iComplexity * 5)
...
float layeredNoise1_2(in vec2 uv, in float sizeMod, in float alphaMod, in int layers, in float animation)
{
    ...
    for (int i = 0; i < layers; i++)
    {
        ...
        //Adding noise with movement
     	noise += noise1_2(uv * size + sin(LOOP_TIME) * animation * 8.0 * MOVEMENT_DIRECTION * MOVEMENT_SPEED + offset) * alpha;
        alpha *= alphaMod;
        size *= sizeMod;
    }
...
{
 	return LOOP_TIME * ANIMATION_SPEED * (hash1_2(uv) - 0.5) * 2.0;   
}
...
float force2 = 0.2*pow(iForce2 / 5.5, 2.0);
vec3 fireParticles(in vec2 uv, in vec2 originalUV)
{
    ...
    vec2 tempUV = uv + (noise2_2(uv * 2.0) - 0.5) * force2;
    tempUV += -(noise2_2(uv * 3.0 + sin(LOOP_TIME)) - 0.5) * 0.07;
...
{ 
    ...
    for (int i = 0; i < layers; i++)
    {
        ...
        bokehUV = (uv * size + LOOP_TIME * MOVEMENT_DIRECTION * MOVEMENT_SPEED) + offset + noiseOffset; 
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float smokeIntensity = layeredNoise1_2(uv * 10.0 + LOOP_TIME*4.0 * MOVEMENT_DIRECTION * MOVEMENT_SPEED, 1.7, 0.7, 6, 0.2);
