...
{
    float n = -mod(time*1.0,-2.); // noise amount
    float iter = 2.0;
    for (int i = 0; i < 8; i++)
    {
...
float map(vec3 p) 
{
	R(p.xz, (iForce2-1.0)/9.0*2.0*pi+time*1.0);
...
{  
	...
	float force = pow(iForce / 7.5, 3.0);
	...
	vec3 rd = normalize(vec3((fragCoord.xy-0.5*iResolution.xy)/max(iResolution.y, iResolution.x), 1.));
	...
	for (int i=0; i<56; i++)
    #else
    for (int i=0; i<10+iComplexity*15; i++)
    #endif
	{
        ...
        */
        ...
        d = cos(time)*uv.x < 0.1 ? abs(d)+0.07 : d;
        #else
        #ifdef LEFT
        d = abs(d)+0.07;
        #endif
		...
		if (d<h) 
		{
            ...
            // emission
            sum += sum.a * vec4(sum.rgb, 0.0) * force / lDist;	
        ...
        uvd.y*=120.;
        uvd.x*=280.;
        d=abs(d)*(.8+0.08*texture(iChannel2,vec2(uvd.y,-uvd.x+0.5*sin(4.*time+uvd.y*4.0))).r);
        #endif 
