...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec3 eye = vec3(0., 0.25+0.25*cos(1.0*LOOP_TIME), -1.);
  ...
  float off = 1.0*LOOP_TIME;
  float h = 0.;
  float z = 0.1;
  float zi = 0.05;
  float power = pow(iForce/5.5, 1.5)+0.2;
  float nbLines = pow(2.+iComplexity,2.);
  for (int i=0; i<nbLines; ++i)
  {
        pos = vec4(p.x, power*0.25*fbm4(.5*vec2(eye.x+p.x, z+off)), eye.z+z, 1.);
        h = (vpmat*pos).y - p.y;
    if (h>lh)
    {
      d = abs(h);
      col = vec3( d<0.005?smoothstep(1.,0.,d*.1):0. );      
            acc += col;
      lh = h;
    }
    z += zi;
