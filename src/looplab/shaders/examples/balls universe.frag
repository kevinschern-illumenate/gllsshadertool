...
#define NUMSTARS iComplexity * 15
#define STARSIZE iForce*0.045
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    time = LOOP_TIME*1.0+69.;
    fragColor = vec4(uv,0.5+0.5*sin(LOOP_TIME),1.0);
    
   
    time *= 2.;
    
    float t = time*pi*0.1;
    COOLCOLOR = vec4(0., 0.1, 0.5, 1.)*0.5+0.5;
    HOTCOLOR = vec4(1., 0., 0.1, 1.)*0.5+0.5;
    ...
    float camHeight = mix(3.5, 0.1, PeriodicPulse(time*1.0, 2.));
