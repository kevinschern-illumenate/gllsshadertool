...
float noiseLayers(in vec3 p) {
    ...
    vec3 t = vec3(0., 0., p.z+LOOP_TIME*1.0*1.5);

    int iter = iComplexity; // Just five layers is enough.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    uv += vec2(sin(LOOP_TIME*1.0*0.5)*0.25, cos(LOOP_TIME*1.0*0.5)*0.125);
    
    // Constructing the unit ray. 
    vec3 rd = normalize(vec3(uv.x, uv.y, 3.1415926535898/8.));

    // Rotating the ray about the XY plane, to simulate a rolling camera.
    float cs = cos(LOOP_TIME*1.0*0.25), si = sin(LOOP_TIME*1.0*0.25);
