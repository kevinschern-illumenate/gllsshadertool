...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    uv *= 0.6 + sin(time*1.0) * 0.4;
    uv = rotate(uv, sin(time*1.0) * 1.0);
    uv += sin(time)*1.0;
    
    float electronTime = time*1.0;
    
    // add some noise octaves
    float a = 0.6, f = 1.0;
    
    for(int i = 0; i < iComplexity; i ++) // 4 octaves also look nice, its getting a bit slow though
    {   
        ...
        if(i > 0)
        {
            ...
            v2 = voronoi(uv * f * 0.5 + 50.0 + electronTime);
